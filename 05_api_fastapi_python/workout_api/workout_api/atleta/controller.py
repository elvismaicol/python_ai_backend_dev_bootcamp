...
async def post(
    ...
    if not centro_treinamento:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, 
            detail=f'A Centro Treinamento {centro_treinamento_nome} não foi encontrado.'
    )
...
async def query(
    ...
    return Page[AtletaOut](items=[AtletaOut.model_validate(atleta) for atleta in atletas], page=offset, limit=limit)
...
async def query(
    id: UUID4, 
    db_session: DatabaseDependency, atleta_up: AtletaUpdate = Body(...)) -> AtletaOut:
    atleta: AtletaOut = (
        await db_session.execute(select(AtletaModel).filter_by(id=id))
    ).scalars().first()
